...
out_path = f'{args.base_folder}/{args.dataset}/gpe/{args.out_filename}'

os.makedirs(f'{args.base_folder}/{args.dataset}/gpe', exist_ok=True)
output_dict_lst = []
...
for data in tqdm(data_lst):
    if 'GPE' in data['answer_entities']:
        gpe_lst = data['answer_entities']['GPE']
        output_dict = {'idx': data['idx'], 'gpes': gpe_lst, 'perturbable': []}
